...
```
```{r percep plot}
percep_plot <- function(weights_df, df, coefs, ttl)
{  
  b0 <- coefs[1]
  b1 <- coefs[2]
  b2 <- coefs[3]
  ...
  plot <- ggplot(weights_df)+
    ...
    geom_point(data=df, mapping=aes(x=x1, y=x2, colour = as.factor(y))) +
    labs(colour = "category", x = "x1", y = "x2")+
    ggtitle(ttl, subtitle = "Red line is decision boundary, blue line is perceptron estimate")
    
  return(plot)
}
```

```{r percep plot animated}
...
plot_3b <- percep_plot(weights_3b, df_3a, coefs_3a, "Question 3B")
plot_3b
...
plot_3c <- percep_plot(weights_3c, df_3c, coefs = coefs_3c, "Question 3C")
plot_3c
...
plot_3d <- percep_plot(weights_3d, df_3d, coefs_3d, "Question 3D")
...
We observe that the larger the dataset, the closer the perceptron decision boundary is to the original. 
```{r 3e1}
b1 <- runif(1, -1, 1) 
b2 <- runif(1, -1, 1) 
b0  <- runif(1, -0.2, 0.2) 
...
plot_3e <- percep_plot(weights_3e, df_3e, coefs_3e, "Question 3E")
